...
/**
 * Class Panel
 * @package backend\themes\basic\widgets
 * Theme Box widget.
 */
class Panel extends Widget
{
    ...
    public function init()
    {
        ...
        if ($this->title !== null || !empty($this->buttons)) {
            // Begin box header
            echo Html::beginTag('div', ['class' => 'box__header']);
            // Box title
            if ($this->title !== null) {
                echo Html::tag('h3', $this->title, ['class' => 'box__title']);
            }
            // Render buttons
            $this->renderButtons();
            // End box header
    ...
    protected function initOptions()
    {
        ...
        $this->bodyOptions['class'] = isset($this->bodyOptions['class']) ? 'box__body ' . $this->bodyOptions['class'] : 'box__body';
        $this->footerOptions['class'] = isset($this->footerOptions['class']) ? 'box__footer ' . $this->footerOptions['class'] : 'box__footer';
    }
    /**
    ...
    protected function initButtons()
    {
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = [
                ...
                'options' => [
                    'class' => 'btn-danger',
                    'title' => Yii::t('default', 'Delete'),
                    'data-confirm' => Yii::t('default', 'Are you sure you want to delete this item?'),
                    'data-method' => 'delete'
                ]
            ];
        }
        if (!isset($this->buttons['batch-delete'])) {
            $this->buttons['batch-delete'] = [
                ...
                'options' => [
                    'id' => 'batch-delete',
                    'class' => 'btn-default',
                    'title' => Yii::t('default', 'Delete selected')
                ]
            ];
        }
    ...
    protected function renderButtons()
    {
        ...
        if ($this->buttonsTemplate !== null && !empty($this->buttons)) {
            // Begin box tools
            echo Html::beginTag('div', ['class' => 'box__header_tools pull-right']);
    ...
    protected function registerClientScripts()
    {
        ...
        if (strpos($this->buttonsTemplate, '{batch-delete}') !== false && $this->grid !== null && isset($this->buttons['batch-delete'])) {
            ...
            $view->registerJs(
                ...
                "if (keys == '') {" .
                "alert('" . Yii::t('default', 'You need to select at least one item.') . "');" .
                "} else {" .
                "if (confirm('" . Yii::t('default', 'Are you sure you want to delete selected items?') . "')) {" .
                "jQuery.ajax({" .
                "type: 'POST'," .
