...
class SiteController extends Controller
{
    ...
    public function actionAddToCart($id, $count = 1)
    {
        ...
        if (isset($cart['products'][$product->id])) {
            ...
            $cart['products'][$product->id]['count'] = $count;
            $cart['products'][$product->id]['getImgPath'] = 'data:image/png;base64,'.$product->images['0'];
        }
        $amount = 0.0;
    ...
    public function actionSearch()
    {
        ...
        $lang = Yii::$app->language;
        if ($lang == 'en')
            $query = Product::find()->where(['like', 'product_en.title', $search])->andWhere(['status' => (string)Product::STATUS_ACTIVE]);
        elseif ($lang == 'kz')
            $query = Product::find()->where(['like', 'product_kz.title', $search])->andWhere(['status' => (string)Product::STATUS_ACTIVE]);
        else
    ...
    public function actionCategorySearch()
    {
        ...
        $categoryIds = Category::find()->where(['parent_id' => (string)$search])->orWhere(['id' => (int)$search])->distinct('id');
        $query = Product::find();
        foreach ($categoryIds as $categoryId)
            $query->orWhere(['category_id' => (string)$categoryId]);

        $query->andWhere(['status' => (string)Product::STATUS_ACTIVE]);
        $pages = new Pagination([
            'totalCount' => $query->count(),
            'pageSize' => 12,
            'forcePageParam' => false,
